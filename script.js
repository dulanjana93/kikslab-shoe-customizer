  ...
  function (error) {
    console.error(error);
      ...
      function handleImageUpload(event) {
        ...
        // Once the file is loaded, create a texture and apply it to the selected part
        reader.onload = function (e) {
          const imgTexture = new THREE.TextureLoader().load(e.target.result);
          
          // Optionally set wrapping and repeat properties for the texture
          imgTexture.wrapS = THREE.RepeatWrapping;
          imgTexture.wrapT = THREE.RepeatWrapping;

          // Create material using uploaded image texture
          const imageMaterial = new THREE.MeshPhongMaterial({
            map: imgTexture,
            shininess: 10 // You can adjust shininess based on your preference
          });

          // Apply the material to the currently selected part (activeOption)
          setMaterial(theModel, activeOption, imageMaterial);
        };
      ...
      function setMaterial(parent, type, mtl) {
        parent.traverse(o => {
          if (o.isMesh && o.nameID != null) {
            if (o.nameID == type) {
              o.material = mtl; // Assign the new material to the selected shoe part
            }
          }
        });
      }
